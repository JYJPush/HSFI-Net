...
class FFM(nn.Module):
    def __init__(self, channels):
        ...
        self.upsampling2 = nn.Upsample(scale_factor=(1, 2, 2), mode='trilinear', align_corners=False)
        self.conv1 = BasicConv3d(channels*3, channels, k=(1, 3, 3), s=(1, 1, 1), p=(0, 1, 1))
    ...
    def forward(self, x1, x2, x3, x4):
        y = self.conv1(torch.cat((x1, x2, x3), 1))
        # SG = self.fc1(x4)
...
class TSYNet(nn.Module):
    def __init__(self, T=32, h=224, w=384, pretrained=None):
        ...
        self.conv3 = nn.Sequential(
            self.upsampling2,
            BasicConv3d(192, 96, k=(2, 3, 3), s=(2, 1, 1), p=(0, 1, 1)),
            self.upsampling2,
            BasicConv3d(96, 64, k=(2, 3, 3), s=(2, 1, 1), p=(0, 1, 1)),
            self.upsampling2,
            BasicConv3d(64, 32, k=(2, 3, 3), s=(2, 1, 1), p=(0, 1, 1)),
            self.upsampling2,
            nn.Conv3d(32, 16, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1), bias=False),
            self.upsampling2,
            nn.ReLU(),
